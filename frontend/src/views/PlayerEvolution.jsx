  ...
  <TiltCard glowColor="#00F0FF" tiltStrength={7}>
    <div className="hud-panel p-4 relative overflow-hidden">
      ...
      <div className="h-1 mt-2 bg-white/5 relative">
        <div className="h-full bg-[#00F0FF]" style={{width: `${clampPercent((v/max)*100)}%`, boxShadow: "0 0 6px #00F0FF"}}/>
      </div>
    </div>
  </TiltCard>
);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-12 gap-4">
        <div className="col-span-12 md:col-span-4 hud-panel p-6 flex flex-col items-center text-center scanline">
          ...
          <div className="w-full mt-4 font-mono text-xs">
            <div className="flex justify-between text-[#8A8A93] mb-1"><span>XP</span><span>{player.xp_into_level}/{player.xp_to_next_level}</span></div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 w-full text-xs font-mono">
            <div className="border border-white/10 p-2 clip-tech text-center"><div className="text-[#8A8A93]">LIFETIME XP</div><div className="text-[#FFB000]">{player.lifetime_xp}</div></div>
            <div className="border border-white/10 p-2 clip-tech text-center"><div className="text-[#8A8A93]">STREAK</div><div className="text-[#FFB000]">{player.streak}D</div></div>
          </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between font-mono text-[10px] mb-1"><span className="text-[#8A8A93]">{k.replace(/_/g, " ")}</span><span className="text-[#FFB000]">{v} XP</span></div>
                      <div className="h-1.5 bg-white/5"><div className="h-full bg-[#FFB000]" style={{width: `${pct}%`, boxShadow: "0 0 4px #FFB000"}}/></div>
                    </div>
                  </motion.div>
                );
              })}
